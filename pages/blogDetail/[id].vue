<script setup lang="ts">
  ...
  //่อ่านข้อมูลจาก API ด้วย useFetch
  const { data: product, pending } =  await useFetch(`https://www.itgenius.co.th/sandbox_api/mrta_flutter_api/public/api/news/${id}`);
  ...
  meta: [
    ...
    {
      name: "Description",
      content: `${product.value.topic},  IT Genius Engineering`,
    },
  ],
});
  ...
  <div class="mb-5" v-if="!pending">
    ...
    <div class="wrapper">
        ...
        <ClientOnly> 
            ...
            <h3 my-5>{{ product.detail }}</h3>
            <p my-5>{{ product.created_at }}</p>
